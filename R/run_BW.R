...
# __OFFICIAL WGWIDE ASSESSMENT__
conf <- loadConf(dat, file = "code/conf/bw/wgwide.cfg")
par <- defpar(dat,conf)
fit <- sam.fit(dat,conf,par)
save(fit, file = "code/model/bw/wgwide_fit.Rdata")

# __Model with predlink__
conf2 <- loadConf(dat, file = "code/conf/bw/withPredVar.cfg")
par2 <- defpar(dat,conf2)
fit2 <- sam.fit(dat, conf2, par2)
save(fit2, file = "code/model/bw/fitWithPredlink.Rdata")

# __variance weighted model run__
load("code/data/bw/weighted.Rdata")
conf3 <- loadConf(dat3, file = "code/conf/bw/wgwide.cfg")
conf3$fixVarToWeight <- 0
par3 <- defpar(dat3,conf3)
fit3 <- sam.fit(dat3,conf3,par3)

#___model with internal variance as external variance_____
load("code/data/bw/weighted_internal.Rdata")
dat4$weight <- 1/dat4$weight^2
conf4 <- loadConf(dat4, file = "code/conf/bw/wgwide.cfg")
conf4$fixVarToWeight <- 1
conf4$keyVarObs[conf4$keyVarObs>=0] <- 0
#conf4$keyVarObs[1:2,] <- 0
par4 <- defpar(dat4,conf4)
fit4 <- sam.fit(dat4, conf4, par4)

conf5 <- loadConf(dat, file = "code/conf/bw/withPredVar.cfg")
conf5$keyVarObs[1,] <- 0
conf5$keyVarObs[2,1:8] <- 1
conf5$predVarObsLink[2,1:8] <- 1
...
ylim = c(0,1)
png("plots/CATCHES.png", width = 1920, height = 1200)
par(mfrow=c(2,2))
...
png("plots/SURVEYS.png", width = 1920, height = 1200)
par(mfrow=c(2,2))
...
ggsave("plots/logobs-vs-logvar_a2.png", width = 1920, height = 1200, unit = "px")
ggplot(IBWSS, aes(x = logobs, y = logv)) +
  geom_point()+#aes(col = agefac))+
  geom_smooth(method = "lm") +
  theme(panel.grid = element_blank())+
...
ggsave("plots/logobs-vs-logvar.png", width = 1920, height = 1200, unit = "px")
...
ggsave("plots/logvar-vs-predvar.png", width = 1920, height = 1200, unit = "px")
...
ggplot(IBWSS, aes(x = logv, y = predv)) +
  ...
  scale_y_continuous(limits =c(0,16), name = "Internally predicted variance from Sam", breaks = seq(0,20,5))+
  geom_abline(intercept = 0, slope = 1, col = 2, lty = 2, lwd = 1)
ggsave("plots/logvar-vs-predvar_a2.png", width = 1920, height = 1200, unit = "px")
